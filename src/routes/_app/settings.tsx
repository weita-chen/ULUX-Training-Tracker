  ...
  return (
    <div>
      ...
      <section className="mt-8 rounded-2xl border border-line bg-surface">
        <Row label="暱稱" value={p?.nickname} onClick={() => { setNick(p?.nickname ?? ""); setNickOpen(true); }} />
      ...
      <DialogFrame open={nickOpen} onOpenChange={setNickOpen}>
        <h2 className="font-display text-xl">暱稱</h2>
        <div className="mt-4 space-y-2">
          <Label>新暱稱</Label>
          <Input value={nick} onChange={(e) => setNick(e.target.value)} />
        </div>
        <Button className="mt-6 w-full" onClick={saveNick} disabled={busy}>
          儲存
        </Button>
      </DialogFrame>

      <DialogFrame open={pinOpen} onOpenChange={setPinOpen}>
