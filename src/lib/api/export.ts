...
export const exportWorkbook = createServerFn({ method: "GET" })
  .middleware([authMiddleware])
  .handler(async ({ context }) => {
    const excelMod = await import("exceljs");
    const ExcelJS = (excelMod as unknown as { default?: { Workbook: new () => import("exceljs").Workbook }; Workbook: new () => import("exceljs").Workbook });
    const Workbook = ExcelJS.default?.Workbook ?? ExcelJS.Workbook;
    const sql = await getSql();
    const profile = await sql<{ ulux_id: string; nickname: string; email: string }>`
    ...
    const styleHeader = (sheet: import("exceljs").Worksheet, cols: number) => {
      const row = sheet.getRow(1);
      row.font = { bold: true, color: { argb: "FFF3F0E6" }, name: "Calibri" };
      row.fill = {
        type: "pattern",
        pattern: "solid",
      ...
      for (let i = 1; i <= cols; i++) {
        sheet.getColumn(i).font = { name: "Calibri", color: { argb: `FF${ink}` } };
      }
    };
    ...
    summary.columns = [
      ...
      { header: "訓練", key: "title", width: 28 },
      { header: "類型", key: "type", width: 14 },
      { header: "時長（分）", key: "dur", width: 12 },
      { header: "備註", key: "notes", width: 40 },
    ];
    for (const s of sessions) {
      ...
      summary.addRow({
        ...
        title: `${typeLabel(s.training_type)}`,
        type: typeLabel(s.training_type),
        dur,
        notes: s.notes ?? "",
      });
    }
    ...
    rec.columns = [
      { header: "日期", key: "date", width: 14 },
      { header: "訓練編號", key: "sid", width: 12 },
      { header: "類型", key: "type", width: 12 },
      { header: "部位", key: "muscle", width: 16 },
      { header: "動作", key: "ex", width: 22 },
      { header: "英文", key: "en", width: 22 },
      { header: "器材", key: "eq", width: 12 },
      { header: "組", key: "set", width: 8 },
      { header: "重量 kg", key: "w", width: 12 },
      { header: "額外重量 kg", key: "aw", width: 14 },
      { header: "次數", key: "reps", width: 10 },
      { header: "時間（秒）", key: "dur", width: 12 },
      { header: "距離 m", key: "dist", width: 12 },
    ...
    for (const e of entries) {
      ...
      rec.addRow({
        date: taipeiDateISO(new Date(s.started_at)),
        sid: e.session_id,
        type: typeLabel(e.training_type),
        muscle: e.muscle ?? "",
        ex: e.name_zh,
        en: e.name_en,
        eq: e.eq_zh ?? "",
    ...
    prSheet.columns = [
      { header: "動作", key: "ex", width: 22 },
      { header: "英文", key: "en", width: 22 },
      { header: "類型", key: "kind", width: 16 },
      { header: "數值", key: "val", width: 12 },
      { header: "次數", key: "reps", width: 10 },
      { header: "日期", key: "date", width: 14 },
    ];
    const prAcc = new Map<
      string,
      { ex: string; en: string; kind: string; val: number; reps: number | null; date: string }
    >();
    for (const e of entries) {
      const s = sessionById.get(e.session_id);
    ...
    stats.columns = [
      ...
      { header: "數值", key: "v", width: 20 },
    ];
    const dates = new Set(sessions.map((s) => taipeiDateISO(new Date(s.started_at))));
    stats.addRow({ k: "帳號 ID", v: profile[0]?.ulux_id ?? "" });
    stats.addRow({ k: "暱稱", v: profile[0]?.nickname ?? "" });
    stats.addRow({ k: "訓練日數", v: dates.size });
    stats.addRow({ k: "訓練場次", v: sessions.length });
    stats.addRow({ k: "動作紀錄列數", v: entries.length });
    stats.addRow({ k: "匯出時間", v: new Date().toISOString() });
    styleHeader(stats, 2);

    const buf = await wb.xlsx.writeBuffer();
    const bytes = buf instanceof ArrayBuffer ? new Uint8Array(buf) : new Uint8Array(buf as ArrayBuffer);
    let binary = "";
    for (let i = 0; i < bytes.length; i++) binary += String.fromCharCode(bytes[i]);
    const base64 = btoa(binary);
    const nick = profile[0]?.nickname ?? "ulux";
    const filename = `ULUX-${nick}-${taipeiDateISO()}.xlsx`;
